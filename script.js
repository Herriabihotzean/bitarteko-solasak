...
function mettreBoutonEnLecture(b){b.classList.add("en-lecture");b.classList.remove("en-pause");b.setAttribute("aria-pressed","true");if(lectureAutomatique)b.scrollIntoView({behavior:"smooth",block:"center"})}
function mettreBoutonEnPause(b){b.classList.remove("en-lecture");b.classList.add("en-pause");b.setAttribute("aria-pressed","false");}
function reinitialiserBoutonActif(){if(!boutonActif)return;boutonActif.classList.remove("en-lecture","en-pause");boutonActif.setAttribute("aria-pressed","false");}
...
retourPresentation.addEventListener("click",()=>{lecteur.pause();if(window.parent!==window)window.parent.postMessage("fermer-le-lecteur",window.location.origin);else window.location.href="presentation.html"});
